//
//  WeNewJiahaoApplyIndexViewController.m
//  We_Doc
//
...
@interface WeNewJiahaoApplyIndexViewController () {
...
@implementation WeNewJiahaoApplyIndexViewController
...
- (UITableViewCell *)tableView:(UITableView *)tv cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([currentJiahao.datesToDemo isEqualToString:@""]) {
        ...
        [l3 setText:currentJiahao.datesToDemo];
    }
...
- (void)viewDidLoad
{
    ...
    // 标题
    self.navigationItem.title = @"加号申请";
    ...
    [refreshButton setTitle:@"获取加号申请列表失败，点击刷新" forState:UIControlStateNormal];
...
- (NSMutableArray *)preworkOnJiahaoList:(id)response {
    ...
    [sourceData sortUsingComparator:^NSComparisonResult(id rA, id rB) {
        return [(WeJiahao *)rA date] < [(WeJiahao *)rB date];
    }];
